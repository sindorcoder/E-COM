...
const [current, setCurrent] = useState(8)
  ...
  return (
     <section className="py-[164px]">
          <h1 className="uppercase text-[35px] font-[600] title text-center" >All Products</h1>
          <div className="container">
               
    <div className="grid grid-cols-4 gap-[35px] mt-[82px]">
     {
          loading ?
          <h1>Loading...</h1> : 
          
          data.slice(0, current).map((product) => {
            return (
              <>
               <div className="max-w-[300px] rounded-[5px] overflow-hidden" key={product.id}>
    ...
    <div className="flex items-center justify-center mt-[60px]">
     <button onClick={() => setCurrent(current + 4)} className="uppercase text-[#40BFFF] border-b-[3px] border-solid border-[#40BFFF] font-[700] text-[18px] leading-[27px] tracking-[0.5px]">Load More</button>
    </div>
          </div>
     </section>
  )
